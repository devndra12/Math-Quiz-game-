...
function getOptions() {
    ...
    var optBtns = document.getElementsByClassName("ansOpt");

    for (let i = 0; i < 4; i++) {
        if (answer > 100) {
            optBtns[i].innerHTML = answer + Math.floor(Math.random() * answer * 0.4);
        } else if (answer > 0 && answer < 100) {
            optBtns[i].innerHTML = answer + Math.floor(Math.random() * answer * 0.6);
        } else {
            optBtns[i].innerHTML = Math.floor(Math.random() * 100);
        }

        if (answer < 0) {
            optBtns[i].innerHTML = "-" + buttons[i].innerHTML;
        }
    }
